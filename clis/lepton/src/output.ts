 ...
 *
 * Forked from apps/cli/src/output.ts WITHOUT the HTTP ApiError dependency — the
 * Lepton operator CLI talks to the chain and the local database, never to the
 * SettleKit HTTP API, so {@link formatError} only handles Error / unknown.
 *
...
/**
 * Format an error for stderr. Errors print their message; anything else is
 * stringified. Returns the string so callers can also use it in tests.
 */
