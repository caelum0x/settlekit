...
Also done (Phase 3 — creator products): **`apps/creator-dashboard`** — a Next.js earnings view in the Statement greenbar-ledger design system (gradient-free, tabular-mono, credit/debit semantics). It is NOT mocked: `lib/data.ts` drives the real domain logic — sources built with `createSource`, every paid access run through `computeRoyaltyDistribution` (so a creator earns their author share *plus* the recursive cut of everything that cites them), and the attribution panel runs `detectReuse` + issues a real signed proof. Pages: **Statement** (lifetime/settled/pending + recent royalty lines), **Sources** (authored vs cited, recursive cut), **Payouts** (settlement sweeps), **Reuse & proofs** (interactive `/api/detect` + worked proof-of-citation), **Wallet**. `next build` green (9 routes); swap `lib/data.ts` for the Pg stores to go live. Public (no auth gate) so it doubles as the demo link.

Also done (RFB 6.01): **`@settlekit/attribution`** — the reuse-detection / proof-of-citation layer the citation toll was designed around. A weighted provenance DAG (`LineageGraph` + `computeAttributionShares`, the citation split expressed as fractions any consumer can use), w-shingling reuse detection (`detectReuse`: does an agent's generated text get grounded in a known source?), and HMAC-signed, expiring, replay-protected proofs-of-citation (`issueCitationProof`/`verifyCitationProof`). Pure + fully tested (30 tests), persisted via `PgLineageStore`/`PgProofStore` over new `lepton_lineage_edges` + `lepton_citation_proofs` tables. Wired live into the **rsshub-citation-toll** sidecar: `POST /attribution/detect` quotes the toll owed for implicit reuse, `POST /attribution/proof` issues a presentable proof, `POST /attribution/verify` checks + consumes its nonce. This closes the loop from *explicit* fetch-tolls to *implicit* reuse-tolls at the agent boundary.
